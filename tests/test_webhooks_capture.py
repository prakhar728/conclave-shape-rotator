"""Tests for /api/webhooks/capture/meeting-completed.
...
def test_unsigned_webhook_accepted_when_secret_unset(app_client: TestClient, monkeypatch):
    ...
    r = app_client.post(
        "/api/webhooks/capture/meeting-completed", json=_event_body()
    )
...
def test_signed_webhook_verified(app_client: TestClient, monkeypatch):
    monkeypatch.setenv("CAPTURE_WEBHOOK_SECRET", "test-secret")
    body = _event_body()
    raw = json.dumps(body).encode()
    sig = "sha256=" + hmac.new(b"test-secret", raw, hashlib.sha256).hexdigest()
    r = app_client.post(
        "/api/webhooks/capture/meeting-completed",
        content=raw,
...
def test_bad_signature_401(app_client: TestClient, monkeypatch):
    monkeypatch.setenv("CAPTURE_WEBHOOK_SECRET", "real-secret")
    body = _event_body()
    raw = json.dumps(body).encode()
...
def test_non_completed_event_ignored(app_client: TestClient, monkeypatch):
    ...
    r = app_client.post(
        "/api/webhooks/capture/meeting-completed", json=body
    )
    assert r.status_code == 202
